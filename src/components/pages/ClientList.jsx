  ...
  const DeleteClient = async (id, email, rowNode) => {
    try {
      const request = await axios.delete(
        `${import.meta.env.REACT_APP_BACKEND_URL}/Auth/delete?email=${email}`,
        {
          headers: {
      ...
      >
                ...
                <input
                  ...
                  onChange={(event) => {
                    if (event.target.checked)
                      ...
                      setSelectedClients(
                        selectedClients.filter((item) => item !== client.email)
                      );
                  }}
                />
              </TableCell>
              <TableCell sx={{ width: "240px" }}>{client.email}</TableCell>
              <TableCell>
                <Button
                  variant="contained"
                  color="error"
        ...
        <Grid>
          <Button
            ...
            onClick={() => {
              if (role == "select") alert("Wybierz rolę!");
              else
          ...
          <Button
            ...
            onClick={() => {
              if (role == "select") alert("Wybierz rolę!");
              else selectedClients.forEach((email) => AssignUserToRole(email));
            }}
          >
            Dodaj rolę
          </Button>
